...
const MobileFooter = () => {
  const account_dealer = JSON.parse(localStorage.getItem("user_md"));
  return (
    <div className="bg-white bg-no-repeat custom-dropshadow footer-bg dark:bg-slate-700 flex justify-between items-center backdrop-filter backdrop-blur-[40px] fixed left-0 w-full z-[9999] bottom-0 py-[12px] px-4">
      {/* <NavLink to="chat">
        {({ isActive }) => (
          <div>
            <span
      ...
      <div className="flex items-center gap-x-2">
        ...
        <span>{account_dealer?.username}</span>
      </div>
      <Button
        text={"Logout"}
        className="text-black-500 hover:bg-slate-200"
        icon={"mdi:sign-out"}
      ></Button>
    </div>
  );
