...
const formValidation = yup
  .object({
    education_name: yup.string().required("Nama wajib diisi"),
  })
  .required();
...
const ModalEducation = () => {
  ...
  const modalState = createStore((state) => state.modal.modalMasterEducation);
  ...
  const { mutate: onSubmit, isLoading } = useMutation({
    mutationFn: async (data) => {
      if (modalItem) {
        const response = await master.updateEducation(
          modalItem?.education_id,
          data
        );
        return response;
      }
      const response = await master.createEducation(data);
      return response;
    },
    onSuccess: (res) => {
      if (res?.meta?.code === 200) {
        ...
        handleModalClose("modalMasterEducation", false, modalItem);
        queryClient.invalidateQueries({ queryKey: ["getEducationList"] });
        reset();
      } else {
        SweetAlert("error", `Education failed to create`, "error");
      }
    },
    onError: (res) => {
  ...
  useEffect(() => {
    if (modalItem) {
      setValue("education_name", modalItem?.education_name);
    } else {
      reset();
    }
  }, [modalItem]);
  ...
  return (
    <Modal
      ...
      onClose={() => handleModalClose("modalMasterEducation", false, modalItem)}
      centered
      title={modalItem ? "Edit Education" : "Add New Education"}
      footerContent={
