...
const formValidation = yup
  .object({
    method_sales_name: yup.string().required("Nama wajib diisi"),
  })
  .required();
...
const ModalTenor = () => {
  ...
  const modalState = createStore(
    (state) => state.modal.modalMasterPoolingSource
  );
  ...
  const { mutate: onSubmit, isLoading } = useMutation({
    mutationFn: async (data) => {
      if (modalItem) {
        const response = await master.updatePoolingSource(
          modalItem?.method_sales_id,
          data
        );
        return response;
      }
      const response = await master.createPoolingSource(data);
      return response;
    },
    onSuccess: (res) => {
      if (res?.meta?.code === 200) {
        SweetAlert(
          "success",
          modalItem
            ? `Pooling Source Edited Successfully`
            : `Pooling Source Created Successfully `,
          "Success"
        );
        ...
        SweetAlert("error", `Pooling Source failed to create`, "error");
      }
    },
    onError: (res) => {
  ...
  useEffect(() => {
    if (modalItem) {
      setValue("method_sales_name", modalItem?.method_sales_name);
    } else {
      reset();
    }
  }, [modalItem]);
  ...
  return (
    <Modal
      ...
      centered
      title={modalItem ? "Edit Pooling Source" : "Add New Pooling Source"}
      footerContent={
      ...
      <div className="grid grid-cols-12 gap-5">
        <div className="col-span-12">
          <Textinput
            ...
            label={"Nama"}
            placeholder="Nama"
            name={"method_sales_name"}
            register={register}
            error={errors.method_sales_name}
          />
        </div>
      </div>
    </Modal>
