...
const formValidation = yup
  .object({
    motor_brand_name: yup.string().required("Nama Merk Motor wajib diisi"),
  })
  .required();

const ModalMotorBrand = () => {
  const {
    control,
    handleSubmit,
    formState: { errors },
  ...
  const modalState = createStore((state) => state.modal.modalMasterMotorBrand);
  ...
  const { mutate: onSubmit, isLoading } = useMutation({
    mutationFn: async (data) => {
      if (modalItem) {
        const response = await master.updateMotorBrand(
          modalItem?.motor_brand_id,
          data
        );
        return response;
      }
      const response = await master.createMotorBrand(data);
      return response;
    },
    onSuccess: (res) => {
      if (res?.meta?.code === 200) {
        SweetAlert(
          "success",
          modalItem
            ? `Motor Brand Edited Successfully`
            : `Motor Brand Created Successfully `,
          "Success"
        );
        handleModalClose("modalMasterMotorBrand", false, modalItem);
        queryClient.invalidateQueries({ queryKey: ["getMotorBrand"] });
        reset();
      } else {
        SweetAlert("error", `Motor Brand failed to create`, "error");
      }
    },
    onError: (res) => {
  ...
  useEffect(() => {
    if (modalItem) {
      setValue("motor_brand_name", modalItem?.motor_brand_name);
    } else {
      reset();
    }
  }, [modalItem]);
  ...
  return (
    <Modal
      ...
      centered
      title={modalItem ? "Edit Motor Brand" : "Add New Motor Brand"}
      footerContent={
      ...
      <div className="grid grid-cols-12 gap-5">
        <div className="col-span-12">
          <Textinput
            ...
            label={"Nama"}
            placeholder="Nama"
            name={"motor_brand_name"}
            register={register}
            error={errors.motor_brand}
          />
        </div>
      </div>
    </Modal>
...
export default ModalMotorBrand;
