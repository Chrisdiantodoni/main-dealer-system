...
import ModalHobbies from "@/components/Modal/Master/ModalHobbies";
...
const Hobbies = () => {
  ...
  const { data, isFetching, refetch, isSuccess, isRefetching } = useQuery({
    queryKey: ["getHobbies"],
    queryFn: async () => {
      const response = await master.getHobbiesList(resultQueryString);
      return response;
    },
  });
  useEffect(() => {
            ...
            <div className="grid grid-cols-2 lg:grid-cols-12 gap-5">
              <div className="col-span-2">
                <Button
                  ...
                  onClick={() => handleModal("modalMasterHobbies", true)}
                />
              </div>
            </div>
          </div>
              ...
              data={data?.data?.data.map((item, index) => ({
                ...
                action: (
                  <Button
                    ...
                    onClick={() =>
                      handleModal("modalMasterHobbies", true, item)
                    }
                  />
                ),
              }))}
