...
import ModalIncome from "@/components/Modal/Master/ModalIncome";
...
const Income = () => {
  ...
  const { data, isFetching, refetch, isSuccess, isRefetching } = useQuery({
    queryKey: ["getIncomeList"],
    queryFn: async () => {
      const response = await master.getIncomeList(resultQueryString);
      return response;
    },
  });
  useEffect(() => {
            ...
            <div className="grid grid-cols-2 lg:grid-cols-12 gap-5">
              <div className="col-span-2">
                <Button
                  ...
                  onClick={() => handleModal("modalMasterIncome", true)}
                />
              </div>
            </div>
          </div>
              ...
              data={data?.data?.data.map((item, index) => ({
                ...
                action: (
                  <Button
                    ...
                    onClick={() => handleModal("modalMasterIncome", true, item)}
                  />
                ),
              }))}
              headers={headers}
