...
import ModalTenor from "@/components/Modal/Master/ModalTenor";
...
const Tenor = () => {
  ...
  const { data, isFetching, refetch, isSuccess, isRefetching } = useQuery({
    queryKey: ["getTenorList"],
    queryFn: async () => {
      const response = await master.getTenorList(resultQueryString);
      return response;
    },
  });
  useEffect(() => {
            ...
            <div className="grid grid-cols-2 lg:grid-cols-12 gap-5">
              <div className="col-span-2">
                <Button
                  ...
                  onClick={() => handleModal("modalMasterTenor", true)}
                />
              </div>
            </div>
          </div>
              ...
              data={data?.data?.data.map((item, index) => ({
                ...
                action: (
                  <Button
                    ...
                    onClick={() => handleModal("modalMasterTenor", true, item)}
                  />
                ),
              }))}
              headers={headers}
