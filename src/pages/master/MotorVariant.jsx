...
import ModalMotorBrand from "@/components/Modal/Master/ModalMotorBrand";
import { dayjsFormatDateTime } from "@/utils/dayjs";
import ModalMasterMotorVariant from "@/components/Modal/Master/ModalMasterMotorVariant";
...
const MotorVariant = () => {
  ...
  const { data, isFetching, refetch, isSuccess, isRefetching } = useQuery({
    queryKey: ["getMotorVariant", resultQueryString],
    queryFn: async () => {
      const response = await master.getVariantList(resultQueryString);
      return response;
    },
  });
  useEffect(() => {
            ...
            <div className="grid grid-cols-2 lg:grid-cols-12 gap-5">
              <div className="col-span-2">
                <Button
                  ...
                  onClick={() => handleModal("modalMasterMotorVariant", true)}
                />
              </div>
            </div>
          </div>
                ...
                deleted_at: (
                  <span>
                    {item?.deleted_at == null
                      ? "-"
                      : dayjsFormatDateTime(item?.deleted_at)}
                  </span>
                ),
                action: (
                  <Button
                    ...
                    onClick={() =>
                      handleModal("modalMasterMotorVariant", true, item)
                    }
                  />
                ),
              }))}
