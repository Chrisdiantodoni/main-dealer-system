...
import ModalMasterCity from "@/components/Modal/Master/ModalMasterCity";
import ModalMasterSubDistrict from "@/components/Modal/Master/ModalMasterSubDistrict";
...
const District = () => {
  ...
  const { data, isFetching, refetch, isSuccess, isRefetching } = useQuery({
    queryKey: ["getSubDistrict", resultQueryString],
    queryFn: async () => {
      const response = await master.getMasterSubDistrictList(resultQueryString);
      return response;
    },
  });
  useEffect(() => {
            ...
            <div className="grid grid-cols-2 lg:grid-cols-12 gap-5 ">
              <div className="col-span-2">
                <Button
                  ...
                  onClick={() => handleModal("modalMasterSubDistrict", true)}
                />
              </div>
            </div>
          </div>
              ...
              data={data?.data?.data.map((item, index) => ({
                ...
                action: (
                  <Button
                    ...
                    onClick={() =>
                      handleModal("modalMasterSubDistrict", true, item)
                    }
                  />
                ),
              }))}
