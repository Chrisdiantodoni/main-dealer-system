...
const City = () => {
  ...
  const { data, isFetching, refetch, isSuccess, isRefetching } = useQuery({
    queryKey: ["getCityList", resultQueryString],
    queryFn: async () => {
      const response = await master.getMasterCityList(resultQueryString);
      return response;
    },
  });
  useEffect(() => {
            ...
            <div className="grid grid-cols-2 lg:grid-cols-12 gap-5 ">
              <div className="col-span-2">
                <Button
                  ...
                  onClick={() => handleModal("modalMasterCity", true)}
                />
              </div>
            </div>
          </div>
              ...
              data={data?.data?.data.map((item, index) => ({
                ...
                action: (
                  <Button
                    ...
                    onClick={() => handleModal("modalMasterCity", true, item)}
                  />
                ),
              }))}
              headers={headers}
