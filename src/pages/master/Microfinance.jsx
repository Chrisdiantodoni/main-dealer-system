...
import ModalMicrofinance from "@/components/Modal/Master/ModalMicrofinance";
...
const Microfinance = () => {
  ...
  const { data, isFetching, refetch, isSuccess, isRefetching } = useQuery({
    queryKey: ["getMicrofinanceList", resultQueryString],
    queryFn: async () => {
      const response = await master.getMicrofinanceList(resultQueryString);
      return response;
    },
  });
  useEffect(() => {
            ...
            <div className="grid grid-cols-2 lg:grid-cols-12 gap-5">
              <div className="col-span-2">
                <Button
                  ...
                  onClick={() => handleModal("modalMasterMicrofinance", true)}
                />
              </div>
            </div>
          </div>
              ...
              data={data?.data?.data.map((item, index) => ({
                ...
                action: (
                  <Button
                    ...
                    onClick={() =>
                      handleModal("modalMasterMicrofinance", true, item)
                    }
                  />
                ),
              }))}
