...
import ModalEducation from "@/components/Modal/Master/ModalEducation";
...
const Education = () => {
  ...
  const { data, isFetching, refetch, isSuccess, isRefetching } = useQuery({
    queryKey: ["getEducationList", resultQueryString],
    queryFn: async () => {
      const response = await master.getEducationList(resultQueryString);
      return response;
    },
  });
  useEffect(() => {
            ...
            <div className="grid grid-cols-2 lg:grid-cols-12 gap-5">
              <div className="col-span-2">
                <Button
                  ...
                  onClick={() => handleModal("modalMasterEducation", true)}
                />
              </div>
            </div>
          </div>
              ...
              data={data?.data?.data.map((item, index) => ({
                ...
                action: (
                  <Button
                    ...
                    onClick={() =>
                      handleModal("modalMasterEducation", true, item)
                    }
                  />
                ),
              }))}
