...
import ModalLeasing from "@/components/Modal/Master/ModalLeasing";
...
const Leasing = () => {
  ...
  const { data, isFetching, refetch, isSuccess, isRefetching } = useQuery({
    queryKey: ["getLeasingList", resultQueryString],
    queryFn: async () => {
      const response = await master.getLeasingList(resultQueryString);
      return response;
    },
  });
  useEffect(() => {
            ...
            <div className="grid grid-cols-2 lg:grid-cols-12 gap-5">
              <div className="col-span-2">
                <Button
                  ...
                  onClick={() => handleModal("modalMasterLeasing", true)}
                />
              </div>
            </div>
          </div>
              ...
              data={data?.data?.data.map((item, index) => ({
                ...
                action: (
                  <Button
                    ...
                    onClick={() =>
                      handleModal("modalMasterLeasing", true, item)
                    }
                  />
                ),
              }))}
