...
function FakturDelivery() {
  ...
  const headers = [
    ...
    {
      title: "JLH SPK",
      key: "samsat_receive_unit_total",
    },
    {
      title: "STATUS",
      key: "samsat_receive_status",
    },
    {
      title: "AKSI",
      key: "action",
  ...
  const { isLoading, data, isSuccess, isRefetching, refetch } = useQuery({
    queryKey: ["getSamsatReceiveList", resultQueryString],
    queryFn: async () => {
      const response = await biro.getSamsatReceive(resultQueryString);
      return response;
    },
  });
              ...
              data={data?.data?.data?.map((item, index) => ({
                ...
                action: (
                  <Button
                    ...
                    text="DETAIL"
                    link={`/biro/samsat-receive/${item?.samsat_receive_id}`}
                  />
                ),
              }))}
            />
